...
public class DownloadTrackingService {
  ...
  public void updateProgress(String downloadUuid, long bytesRead) {
    UserDownload download = downloadTrackingRepository.updateDownloadProgress(downloadUuid, bytesRead);

    if (download.getBytesRead() >= download.getTotalBytes()) {
      log.info("Download {} completed for user {} from library {}, average speed: {} bytes/s",
          downloadUuid, download.getUser().getUsername(), download.getLibrary().getName(),
          download.getTotalBytes() / (System.currentTimeMillis() - download.getStartedAt().toEpochMilli()) * 1000);
    }
  }
